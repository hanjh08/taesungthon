...
const detailResolveButton = document.querySelector(".detail-resolve-button");
...
function openCreateModal() {
  editingItemId = null;
  editingItemImage = "";
  document.querySelector("#register-title").textContent = "분실물 / 습득물 등록";
...
function openDetail(index) {
  ...
  detailStatusBadge.textContent = status === "resolved" ? "해결됨" : "진행 중";
  detailStatusBadge.classList.toggle("resolved", status === "resolved");
  ...
  detailAuthor.textContent = item.author || "익명";
  detailCreated.textContent = formatCreatedAt(item.createdAt);
  detailDescription.textContent = item.description || "등록된 설명이 없습니다.";
  detailResolveButton.disabled = status === "resolved";
  detailResolveButton.textContent = status === "resolved" ? "이미 해결됨" : "해결됨으로 변경";
...
function verifyPassword(item) {
  const inputPassword = prompt("게시물 비밀번호를 입력해주세요.");
...
function openEditModal(item) {
  ...
  form.elements.type.value = item.type || "lost";
  nameInput.value = item.name || "";
  categoryInput.value = item.category || "기타";
  ...
  if (editingItemImage) {
    photoPreview.src = editingItemImage;
  ...
  document.querySelector("#register-title").textContent = "게시물 수정";
...
async function resolveCurrentItem() {
  ...
  if (!item || !verifyPassword(item)) {
    return;
  }

  try {
    detailResolveButton.disabled = true;
    detailResolveButton.textContent = "변경 중...";
    ...
    console.error("상태를 변경하지 못했습니다.", error);
    alert("해결됨으로 변경하지 못했어요. Firebase 규칙을 확인해주세요.");
    detailResolveButton.disabled = false;
    detailResolveButton.textContent = "해결됨으로 변경";
  }
}
...
detailResolveButton.addEventListener("click", resolveCurrentItem);
...
form.addEventListener("submit", async (event) => {
  ...
  submitButton.textContent = editingItemId ? "수정 중..." : "등록 중...";

  try {
    const imageUrl = uploadedFile ? await uploadItemImage(uploadedFile) : editingItemImage || fallbackImage;
    const itemData = {
      type,
      image: imageUrl,
    ...
    alert("게시물을 저장하지 못했어요. Firestore/Storage 규칙을 확인해주세요.");
  } finally {
    submitButton.disabled = false;
    submitButton.textContent = editingItemId ? "수정하기" : "등록하기";
  }
});
